...
router.post("/message/senduser/:senduserid/targetuser/:targetid/new", (req, res) => {
    ...
    async function getUser() {
        try {
            ...
            newMessage = [{messageAuthor: messageAuthor.username, messageTarget:messageTarget.username, content, messageAuthorId, messageTargetId}]
     ...
     async function makeMessage(newMessage) {
        try {
            ...
            await User.findOneAndUpdate({ _id: messageAuthorId }, { $push:{ messages: newlyCreated }}, {upsert: true}).sort('-createdDate')
            await User.findOneAndUpdate({ _id: messageTargetId }, { $push:{ messages: newlyCreated }}, {upsert: true}).sort('-createdDate')
